...
| Path | Tier |
|---|---|
| [`app.js`](app.js) | Express bootstrap (security middleware, CORS, routes). |
| [`app_server/`](app_server) | Customer-facing site (HBS views, controllers, Mongoose model). |
| [`app_api/`](app_api) | REST API (auth, trips, reservations, middleware, query helper). |
| [`travlr-admin/`](travlr-admin) | Angular 21 admin SPA. |
| [`test/`](test) | Unit tests (Node's built-in test runner). |
| [`docs/`](docs) | Capstone ePortfolio source (Jekyll). |
...
| `EMAIL_USER` / `EMAIL_PASS` | no | Gmail App Password for reservation confirmation email. If unset, emails are logged to stdout. |
...
- `helmet` sets defensive HTTP headers. CSP is left off until the legacy inline styles in `public/*.html` are migrated.
- CORS is allowlisted via `CORS_ORIGIN` rather than wide-open.
...
Runs three suites under Node's built-in test runner:
